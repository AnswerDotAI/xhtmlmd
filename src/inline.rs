...
fn parse_inner(src: &str, ctx: &InlineContext<'_>, depth: usize) -> Vec<Inline> {
    ...
    let bracket_matches = bracket_matches(src);
    let mut out = Vec::new();
    ...
    while i < src.len() {
        ...
        if starts(src, i, "![") || starts(src, i, "[") {
            if let Some((item, next)) = link_or_span(src, i, ctx, depth, &bracket_matches) {
                flush(&mut out, &mut text);
                i = push_with_attrs(&mut out, item, src, next, ctx);
                continue;
            }
...
fn link_or_span(
    ...
    let close = bracket_matches.get(open).copied().flatten()?;
...
fn angle_or_html(src: &str, i: usize) -> Option<(Inline, usize)> {
    ...
    if looks_like_html(inside) {
        return Some((Inline::Html(src[i..end + 1].to_string()), end + 1));
    }
    None
}
...
fn bracket_matches(src: &str) -> Vec<Option<usize>> {
    let mut matches = vec![None; src.len()];
...
fn is_email(s: &str) -> bool {
    ...
    at > 0 && s[at + 1..].contains('.') && !s.contains(char::is_whitespace)
}
...
fn is_intraword(src: &str, i: usize) -> bool {
    prev_char(src, i).is_alphanumeric()
        && i + 1 < src.len()
        && next_char(src, i + 1).is_alphanumeric()
}
