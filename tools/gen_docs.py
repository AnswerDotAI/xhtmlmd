...
SAMPLE_MD = ROOT / "docs" / "sample.md"
SAMPLE_HTML = ROOT / "docs" / "sample.html"


def main(argv=None):
    p = argparse.ArgumentParser(description="Generate docs/sample.html from docs/sample.md.")
    p.add_argument("--check", action="store_true", help="fail if generated docs differ from checked-in files")
    args = p.parse_args(argv)
    html = to_xhtml(SAMPLE_MD.read_text(encoding="utf-8"))
    if args.check:
